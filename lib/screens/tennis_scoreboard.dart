...
class _TennisScoreboardState extends State<TennisScoreboard> {
  ...
  Widget _playerColumn({required bool isA}) { final name = isA ? _teamAName : _teamBName; final points = _pointLabel(isA ? _pointsA : _pointsB, isA ? _pointsB : _pointsA); final games = isA ? _gamesA : _gamesB; final sets = isA ? _setsA : _setsB; final color = isA ? const Color(0xFF3B8D78) : const Color(0xFFB2784B); return Expanded(child: GestureDetector(onTap: () => _addPoint(isA), behavior: HitTestBehavior.opaque, child: Container(margin: const EdgeInsets.all(8), decoration: BoxDecoration(color: color.withOpacity(.22), borderRadius: BorderRadius.circular(24), border: Border.all(color: color.withOpacity(.7))), child: SafeArea(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [InkWell(onTap: () => _rename(isA), child: Padding(padding: const EdgeInsets.all(6), child: Text(name.toUpperCase(), style: const TextStyle(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.w800, letterSpacing: 1.5)))), const SizedBox(height: 8), Text(points, style: const TextStyle(color: Colors.white, fontSize: 76, fontWeight: FontWeight.w800)), const SizedBox(height: 10), Text('GAMES  $games', style: const TextStyle(color: Colors.white70, fontSize: 14, letterSpacing: 1)), Text('SETS  $sets', style: const TextStyle(color: Colors.white70, fontSize: 14, letterSpacing: 1)), const SizedBox(height: 14), const Text('Tap anywhere to score', style: TextStyle(color: Colors.white54, fontSize: 12))])))); }
  @override Widget build(BuildContext context) { return Scaffold(appBar: AppBar(title: const Row(children: [Icon(Icons.sports_tennis, color: Color(0xFF9BBE62)), SizedBox(width: 8), Text('Tennis match')]), actions: [IconButton(icon: const Icon(Icons.refresh), tooltip: 'Reset match', onPressed: _reset)]), body: Row(children: [_playerColumn(isA: true), _playerColumn(isA: false)])); }
}
